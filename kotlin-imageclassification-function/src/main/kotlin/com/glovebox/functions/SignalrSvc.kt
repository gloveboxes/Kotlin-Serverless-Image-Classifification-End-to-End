...
class SignalrSvc {
    ...
    private fun deviceState(): List<ImageProbabilityEntity> {
        val partitionFilter = TableQuery.generateFilterCondition("PartitionKey", QueryComparisons.EQUAL, partitionKey)
        val partitionQuery = TableQuery.from(ImageProbabilityEntity::class.java).where(partitionFilter)
        return deviceStateTable.execute(partitionQuery).sortedWith(compareBy(String.CASE_INSENSITIVE_ORDER, { it.tagName })).toList()
    }
}